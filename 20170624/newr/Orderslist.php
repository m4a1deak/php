...
if ($_POST['button2']){
	...
	$id=$_GET['id'];
		...
		if ($orders['issend']==0){
			...
			echo "<script language=javascript>alert('该订单发货已发货.');window.location.href='?'</script>";
		}
...
<table class="table table-bordered table-striped table-hover" style=" margin-bottom:1px;">
      <tr>
        <td height="10" colspan="12" align="center">订单详情</td>
      </tr>
      <tr>
      ...
      <?php
	  	$pagesize = 20; //设置每页记录数 
	  	$sql = "SELECT * FROM `orders1` WHERE  bid='".$_SESSION['ID']."'"." ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `orders1` WHERE  bid='".$_SESSION['ID']."'"." ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by id desc limit ".$start.",".$pagesize;
      //	print_r($_SESSION);
		if($query = mysql_query($sql)){
      ...
      <tr>
        ...
        -->
         <td align="center"><?php if($row['issend']==0){?>未发货<?php }else if($row['issend']==1){ ?>已发货<?php }else if($row['issend']==2){?>已退款<?php }else if($row['issend']==3){?>已收货<?php }else if($row['issend']==4){?>交易完成<?php }?></td>
        <td align="center"><a href="ordercontent.php?oid=<?=$row['id']?>&page=<?=$p?>" style="text-decoration:none">查看</a>
        <?php if($row['issend']==0){?>
        <input name="button2" type="submit"  class="button_green" id="button2" value="发货" onClick="{if(confirm('您确定要发货吗?')){this.document.selform.submit();return true;}return false;}" ID=<?=$row['id']?>/>
