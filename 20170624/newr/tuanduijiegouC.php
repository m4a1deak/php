...
if ($_GET['action']=="admin"){
	...
	if ($_POST['submin']){
		if ($chaus=getMemberbyNickName3($_POST['nickname'])){
			if(checkisppath3($_SESSION['ID'],$chaus['id'])){
				$ID=$chaus['id'];
			}else{
		...
		if ($_GET['ID'] == null){
			...
			if(checkisppath3($_SESSION['ID'],$_GET['ID'])){
				$ID=$_GET['ID'];
			}else{
				echo "<script language=javascript>alert('您的网络中不存在该成员.');window.location.href='?'</script>";		
			}
...
$benren=getMemberbyID4($_SESSION['ID']);		//$benren是登录的会员
$member=$benren;
$ull=ulevel($benren['ulevel']);
	...
	if ($benren['uid']==$ID) {
		...
		$us=getMemberbyID4($ID);
		
	}else {
		
		$us=getMemberbyID44($ID);
	}
	...
	//A区
	if ($p1=getFatherManbyFidAndTreeplace3($UserID,1)){
	...
	//B区
	if ($p2=getFatherManbyFidAndTreeplace3($UserID,2)){
	    if($benren['ispay']>=0){
	...
	//C区
	if ($p3=getFatherManbyFidAndTreeplace3($UserID,3)){
		if($benren['ispay']>=0){
	...
	//d区
	if ($p4=getFatherManbyFidAndTreeplace3($UserID,4)){
		if($benren['ispay']>=0){
	...
	//E区
	if ($p5=getFatherManbyFidAndTreeplace3($UserID,5)){
		if($benren['ispay']>=0){
