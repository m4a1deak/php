...
// 	}
	if($passquestion=='--请选择密保问题!--'){
	
		$zhuce=false;
		echo "<script language=javascript>alert('请选择密保问题.');window.location.href='zhuce_biaodan.php?nickname=".$FatherName."&tid=".$TreePlace."'</script>";
	}
	if($passanswer==''){
	
		$zhuce=false;
		echo "<script language=javascript>alert('请选择密保答案.');window.location.href='zhuce_biaodan.php?nickname=".$FatherName."&tid=".$TreePlace."'</script>";
	}
	if($UserTel==''){
	
		$zhuce=false;
		echo "<script language=javascript>alert('请填写手机号码.');window.location.href='zhuce_biaodan.php?nickname=".$FatherName."&tid=".$TreePlace."'</script>";
	}
...
// 	}
	...
	$sql = "SELECT id FROM `member` where fatherid=".$FatherID." and treeplace=1";
	
	$you=getOne($sql);

	
	if($you['id']==null and $TreePlace==2){
		$zhuce=false;
...
// 							$or['goodid']=$row['id'];
// 							$or['goodname']=$row['goodsname'];
// 							$or['price']=$row['price'];
// 							$or['num']=$num;
// 							$or['jine']=$num*$row['price'];
// 							$or['date']=now();
// 							$or['bid']=$bdid;
// 							$or['bnickname']=$bdName;
// 							add_insert_cl('orders1',$or);
// 						}
// 					}
// 				}
// 			}
