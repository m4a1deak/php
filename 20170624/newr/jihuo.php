				...
				if ($us['bdid']==$me['id']){
					if ($us['ispay']==0){
						if ($me['zsq']>=$us['lsk']){
									...
									$bonus_cl->bonus_laiyuan($me['id'],$me['nickname'],$id,$us['nickname'],13,-$us['lsk'],"报单币");
...
<thead>
      ...
      <tr>
        <td height="10" colspan="5" align="center">奖金：<?=$my['mey']?></td>
...
<tbody> 
       <?php
	  	$pagesize = 10; //设置每页记录数 
	  	$sql = "SELECT id,nickname,username,rname,fathername,lsk,ulevel,usertel,rdt,reid,bdid FROM `member` WHERE  bdid=".$_SESSION['ID']."  and ispay=0 ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT id,nickname,username,rname,fathername,ulevel,lsk,usertel,rdt,reid,bdid FROM `member` WHERE  bdid=".$_SESSION['ID']."  and ispay=0 ".$_SESSION['Search']."  ".$_SESSION['SearchTime']." ORDER BY rdt asc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
