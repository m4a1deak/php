...
$sql = "select * from goods where lx =1 order by id";
...
<font color="#FF0000" size="4"><font size="3">&nbsp;填写说明</font><strong>：</strong></font>
  <p class="style3">一、如果您注册成为会员管理会员，请您立即付款，付款金额在您注册结束后显示，如不及时付款，将影响您的业务进程，本会员管理会员系统将删除您的注册信息，谢谢合作！</p>
  <p class="style3">二、标有"(<font color="#FF0000">*</font>)"符号的栏目必须填写清楚。一定要明确推荐人姓名以及编号，不知推荐人编号的则由会员管理会员系统补填。</p>
	...
	//调用插件
		...
		$(function(){
			...
			$("#province").change(function(){
				...
				$.ajax({
					type:"get",
					url:"../newr/diqu.php?province="+this.value,
					dataType:"text",
					success:function(a){
						$("#city").html(a);
						$("#county").html(a);
			...
			$("#city").change(function(){
				$.ajax({
					type:"get",
					url:"../newr/diqu.php?city="+this.value,
					dataType:"text",
					success:function(a){
						$("#county").html(a);
					}
...
//	UserID=document.form1.UserID.value;
	...
	if(formName=="passquestion"){
		if(passQuestion.length == 0){
			document.getElementById('passQuestionlabel').innerText="请输入密保问题";
		}else{
	...
	if(formName=="passanswer"){
		if(passanswer.length == 0){
			document.getElementById('passanswerlabel').innerText="请输入密保答案";
		}else{
...
<table  width="100%" height="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="form-horizontal">
    ...
    <?php
    $sql="SELECT id,isbd,bdname,bdlevel FROM member where nickname='".$_SESSION['nickname']."'";
    $uss = getOne($sql);
    if($uss['isbd']==2 && $us['bdlevel']==1){$admin=$_SESSION['nickname'];}else {$admin=$uss['bdname'];}
...
&nbsp;<span id="span">(*)</span>&nbsp;<label id="bdNamelabel"/>
  ...
  <tr style="display:none">
    ...
    <td align="left"><select name="passquestion" id="passquestion" >
    <!--  
										        ...
										        <option value="您的最喜欢的书是？">您的最喜欢的书是？</option>
										        <option value="您的最喜欢的电影是？">您的最喜欢的电影是？</option>
										        <option value="您的最喜欢的颜色？">您的最喜欢的颜色？</option>
                                         </select>  </td>
  </tr>
  <tr style="display:none">
    <td align="right">密码安全答案：</td>
    <td align="left"><input name="passanswer" type="text" id="passanswer" onBlur="Check('passanswer');" value="168" maxlength="50"/>
  ...
  <tr >
    ...
    <td align="left"><input name="UserTel" type="text" id="UserTel"  onblur="isPhoneNum(this);" value="168" maxlength="20"/>
