...
if($_POST['button3']){
	...
	if($_POST['UID']==""){
		...
		$lsk=$dianzi+$daijin;
//		$_bonus_cl->b2bonus($member['id'], $member['nickname'], $member['reid'], $lsk);
//		$FileID=store($member['id'],$arr,$member,$member_update,$daijin,$dianzi);
		$FileID=store($member['id'],$arr,$member,$username,$useraddress,$usertel);
...
<!--         <td align="center" >大小杯</td>-->
        <td align="center" >查看详细</td>
 		...
 		<?php
	  	...
	  	$sql = "SELECT * FROM `goods` WHERE isdisplay=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `goods` WHERE isdisplay=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by shunxu,id limit ".$start.",".$pagesize;
