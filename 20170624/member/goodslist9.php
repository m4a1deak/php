	...
	$gwb=0;$pv=0;
		...
		foreach ((array)$cheuid_arr as $id)
		{
			...
			if(is_numeric($num) && $num>0){
				$arr=array("id"=>$goods['id'],"goodsname"=>$goods['goodsname'],"num"=>$num,"price"=>$goods['price'],"lx"=>$goods['lx'],"pv"=>$goods['pv']);
		...
		if($_SESSION['shoppingcart']<>""){
				...
				if($_POST['name']!=$member['username']){
				    $orders['username']=$_POST['name'];
				}
				$orders['usertel']=$member['usertel'];
				if($_POST['usertel']!=$member['usertel']){
				    $orders['usertel']=$_POST['usertel'];
				}
				$orders['useraddress']=$member['useraddress'];
				if($_POST['address']!=$member['useraddress']){
				    $orders['useraddress']=$_POST['address'];
				}
				...
				if ($email['ddtzadmin']==1){
					...
					$system=$system_cl->system_information(1);
					$name=$system['duanxinzhanghao'];
					$pwd  =$system['duanxinmima'];
					$tel  =$member['usertel'];
					//$msg  =rawurlencode("您在维力多购物已成功,请注意查收;您的账号是：".$member['nickname'].".追求天然,缔造健康.");
					//$res = file_get_contents("http://sms.4006555441.com/webservice.asmx/mt?Sn=$name&Pwd=$pwd&mobile=$tel&content=$msg");
					$b=new bonus_class();
					$b->jicha($member['id'],$member['nickname'],$pv);
					$b->dianfanli($member['id'],$pv);
					comsumptionyeji($member['id'],$pv);
					mysql_query("call xiaofeilevel()");
					alert("商品购买成功","Orders.php");
				}else{
					alert("您的积分余额不足,请充值.","?");
				}	
			}
...
function jiage(numname,pic){
	...
	if(!reg.test(num)){
		...
		var zong=num*price;
	var e=document.getElementsByName("UID[]");
	...
	for (var i=0;i<e.length;i++) {
		...
		if (e[i].checked==true){
			var jiage=document.getElementById(e[i].value).value;
			var shuangliang=document.getElementById(e[i].value+"num").value;
...
<body>
<form name="form1" method="post" action="comsumption.php?yemian=goodslist9.php" onsubmit=" return check();">
...
<table width="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="table1">
        ...
        <td align="center" >查看详细</td>
 		...
 		<?php
	  	...
	  	$sql = "SELECT * FROM `goods` WHERE isdisplay=1 and lx=4 ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		if($query = mysql_query($sql)){
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `goods` WHERE isdisplay=1 and lx=4 ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by shunxu,id limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
			while ($row=mysql_fetch_array($query)){
			?>	
      <tr>
        ...
        <td align="center" ><input name="<?=$row['id']?>num" id="<?=$row['id']?>num" type="text" value="0" size="4" onfocus="if(this.value==0)this.value='';" onblur="if(this.value=='')this.value=0;" onKeyUp="jiage(this,<?=$row['id']?>);"><input name="<?=$row['id']?>" id="<?=$row['id']?>" type="hidden" value="<?=$row['price']?>"></td>
