...
<table width="100%" height="100%" border="0">
  <tr valign="top">
    <td colspan="3"><table width="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="table1">
      ...
      <?php
	  	$pagesize = 20; //设置每页记录数 
		$zhou=date("Y-m-d H:i:s",mktime(23,59,59,date("m"),date("d")-7,date("Y")));
	  	$sql = "SELECT * FROM `member` WHERE reid=".$_SESSION['ID']." and  pdt<'$zhou' and recount<2";
	//	echo $sql;
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `member` WHERE reid=".$_SESSION['ID']." and pdt<'$zhou' and recount<2 order by pdt limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
