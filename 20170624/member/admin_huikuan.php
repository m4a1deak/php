...
<table width="100%" cellpadding="3" cellspacing="0" border="0" align="center" class="table1">
      ...
      <?php
	  	$pagesize = 20; //设置每页记录数 
	  	$sql = "SELECT * FROM `huikuan` WHERE rid=".$_SESSION['ID']." ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `huikuan` WHERE rid=".$_SESSION['ID']." ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by hdate desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
      ...
      <tr>
        ...
        <table width="100%" border="0">
          <tr>
            <td align="left" ><input name="button" class="button_green" type="submit" class="btn1" id="button" value="确认汇款" onClick="{if(confirm('您确定要进行确认吗?')){this.document.selform.submit();return true;}return false;}">
<!--            <input name="button4" class="button_red" type="submit" class="btn3" id="button4" value="删除记录" onClick="{if(confirm('您确定要删除该记录吗?')){this.document.selform.submit();return true;}return false;}"></td>-->
