...
<table width="100%" cellpadding="3" cellspacing="0" border="0" align="center" class="table1">
      ...
      <tr>
        <td height="10" colspan="7" align="center"> 报 单 记 录</td>
      </tr>
      <tr>
      ...
      <?php
	  	$pagesize = 20; //设置每页记录数 
	  	$sql = "SELECT * FROM `lsbd` WHERE 1=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `lsbd` WHERE 1=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by bdate desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
