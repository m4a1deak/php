...
$cheuid_arr = $_POST['UID'];
	...
	{
		$tuidan=que_select_cl('tuidan',$id);
		if ($tuidan['isgrant']==0){
			$us=que_select_cl('member',$tuidan['uid']);
			$us_update['islock']=1;
			edit_update_cl('member',$us_update,$us['id']);
			$cc_xiugai['isgrant']=1;
    		edit_update_cl('tuidan',$cc_xiugai,$id);
		}
	}
	echo "<script language=javascript>alert('退单完成,会员已被锁定.');window.location.href='?'</script>";
}
...
<table width="100%" cellpadding="3" cellspacing="0" border="0" align="center" class="table1">
      ...
      <tr >
        <td colspan="8" align="center">退 单 管 理</td>
      </tr>
      <tr>
        ...
        <td align="center" class="td1">退单金额</td>
        <td align="center" class="td1">手续费</td>
         <td align="center" class="td1">实际金额</td>
        <td align="center">申请时间</td>
        <td align="center">审核状态</td>
        </tr>
      <?php
	  	$pagesize = 20; //设置每页记录数 
	  	$sql = "SELECT * FROM `tuidan` WHERE 1=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `tuidan` WHERE 1=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by cdate desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
      ...
      <tr>
        ...
        <td align="center"><?=$row['jine']-$row['jine']*0.1?></td>
        <td align="center"><?=$row['cdate']?></td>
        <td align="center"><?php if ($row['isgrant']==1){?>已审核<?php }else{?> <font color="#FF0000">未审核</font><?php }?></td>
      </tr>
      <?php
			}
		}
      ...
      <tr>
        ...
        <table width="100%" border="0">
          <tr>
            <td align="left"><input name="button" type="submit" class="btn1" id="button" value="退单确认" onClick="{if(confirm('您确定要进行退单确认吗?')){this.document.selform.submit();return true;}return false;}">
