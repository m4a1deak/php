...
if ($_POST['button2']){
	...
	foreach ((array)$cheuid_arr as $id)
	{
		...
		$member=que_select_cl('member',$orders['uid']);
			...
			$upmember['zsq']=$member['zsq']+$orders['jine'];
...
<body>
    ...
    <input type="button" name="dayin" id="dayin" class="btn1" value="未发货详情" onClick="window.location.href='excel.php?table=orders0'">
    <input type="button" name="dayin" id="dayin" class="btn1" value="已发货详情" onClick="window.location.href='excel.php?table=orders1'">
    <input type="button" name="dayin" id="dayin" class="btn1" value="收货订单详情" onClick="window.location.href='excel.php?table=orders3'">
    </div>
    <div id="tablit">
    <dl>
        <dt></dt>
...
<table width="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="table1">
      ...
      <?php
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `orders1` WHERE issend=0 and isqr=1 ".$_SESSION['Search']." and date>='".$_SESSION['TimeStart']."' and date<='".$_SESSION['TimeEnd']."' order by id desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
...
<table width="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="table1">
      ...
      <?php
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `orders1` WHERE issend=1 ".$_SESSION['Search']." and sdate>='".$_SESSION['TimeStart']."' and sdate<='".$_SESSION['TimeEnd']."' order by id desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
		while ($row=mysql_fetch_array($query)){
	  ?>
      <tr>
      ...
      <tr>
        ...
        <table width="100%" border="0">
          <tr>
            <td align="left">
            <input name="button3" style="display:none;" type="submit" class="btn3" id="button3" value="退款" onClick="{if(confirm('您确定要退款吗?')){this.document.selform.submit();return true;}return false;}">
...
<table width="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="table1">
      ...
      <?php
	  	...
	  	$pagesize = 20; //设置每页记录数 
	  	$sql = "SELECT * FROM `orders1` WHERE issend=3 ".$_SESSION['Search']." and date>='".$_SESSION['TimeStart']."' and date<='".$_SESSION['TimeEnd']."'";
		if($query = mysql_query($sql)){
	  		$sum = mysql_num_rows($query); //计算总记录数 
		}else{
			$sum=0;	
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `orders1` WHERE issend=3 ".$_SESSION['Search']." and date>='".$_SESSION['TimeStart']."' and date<='".$_SESSION['TimeEnd']."' order by id desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
		while ($row=mysql_fetch_array($query)){
	  ?>
      <tr>
