...
$cheuid_arr = $_POST['UID'];
	...
	{	$ti=que_select_cl('tixian',$id);
		action::record("提现确认", $ti['uid'], $_SESSION['adminid'],"确认提现");
...
$cheuid_arr = $_POST['UID'];
	foreach ((array)$cheuid_arr as $id)
	{
		...
		if($tixian['isgrant'] == 0){
			...
			action::record("删除提现记录", $tixian['uid'], $_SESSION['adminid'],"删除记录");
			...
			echo "<script>alert('已提现记录不能删除');</script>";
		}
...
<table width="100%" cellpadding="3" cellspacing="0" border="0" align="center" class="table1">
      <tr>
        ...
        <td height="4" align="center"><input type="button" name="dayin" id="dayin" class="btn1" value="导出表格" onClick="window.location.href='excel.php?table=tixian'"></td>
      </tr>
      <tr>
      ...
      <tr>
        <td height="10" colspan="13" align="center"> 提 现 管 理</td>
      </tr>
      <tr>
      ...
      <?php
	  	$pagesize = 20; //设置每页记录数 
	  	$sql = "SELECT * FROM `tixian` WHERE 1=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `tixian` WHERE 1=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by tdate desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
      ...
      <tr>
        ...
        <table width="100%" border="0">
          <tr>
            <td align="left"><input name="button" type="submit" class="btn1" id="button" value="提现确认" onClick="{if(confirm('您确定要进行提现确认吗?')){this.document.selform.submit();return true;}return false;}">
