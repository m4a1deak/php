...
class system_class{
	...
	function system_right(){
		...
		$_m=date("m");
		$_d=date("d");
		$sql="SELECT * FROM `member` where ispay>0 and year(pdt)=".$_y." and month(pdt)=".$_m." and day(pdt)=".$_d." and id<>1";
		$_xyejiday=0;
		if ($query=mysql_query($sql)){
			$_xhyday=mysql_num_rows($query);
			while($row=mysql_fetch_array($query)){
				$_xyejiday=$_xyejiday+$row['dan'];
			}	
		}else{
			$_xhyday=0;
		}
		
		$sql="SELECT * FROM `member` where ispay>0 and year(pdt)=".$_y." and month(pdt)=".$_m." and id<>1";
		$_xyejimonth=0;
		if ($query=mysql_query($sql)){
			$_xhymonth=mysql_num_rows($query);
			while($row=mysql_fetch_array($query)){
				$_xyejimonth=$_xyejimonth+$row['dan'];
			}	
		}else{
			$_xhymonth=0;
		}
		
		$sql="SELECT * FROM `member` where ispay>0 and year(pdt)=".$_y." and id<>1";
		$_xyejiyear=0;
		if ($query=mysql_query($sql)){
			$_xhyyear=mysql_num_rows($query);
			while($row=mysql_fetch_array($query)){
				$_xyejiyear=$_xyejiyear+$row['dan'];
			}	
		}else{
			$_xhyyear=0;
		}
