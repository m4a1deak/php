...
$sql = "SELECT bh from ".TABLE_USER." where userdir='$userdir'   limit 1";
	if(getnum($sql)==0){ echo  'not exist user...';exit;}
		...
		$langpath = $row['path']; $langfile = $row['langfile'];
		...
		if($langfile<>'')  $langfile = 'lang_'.$langfile;
...
//if(ENABLEMULTILANG=='y') $langfileV = 'lang_'.LANG;
if($langfile<>'') $langfileV = $langfile;
else $langfileV = 'lang_'.LANG;

$langname = 'component/lang/'.$langfileV.'.php';
