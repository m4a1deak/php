...
function reqfile($file,$filename) { 
     if (is_file($file))    require_once $file; 
     else { echo '<p style="background:red;color:#fff">'.$filename.' is not exist!文件不存在</p>';}
}
...
function detail_linkmore(){
 ...
 if($linkmore<>''){
     echo '<div class="detaillinkmore ptb10 dmbtn moresm"><a class="more" '.linktarget($linkmore).' href="'.$linkmore.'">'.$linkmoretitlev.'</a></div>';
 }
...
function detail_downloadurl($downloadurl){
   if($downloadurl<>'') echo '<div class="detaillinkurl ptb10 dmbtn moresm"><a class="more" href="'.$downloadurl.'" target="_blank"><i class="fa fa-download"></i> 资料下载</a></div>';
...
function cus_columnsv($cus_columns){ //block判断列数
		$css_gridcol = 'colfull';
...
function getCssCommon(){
  ...
  echo '<link href="'.TPLBASEPATH.'assets/css/DMcompress.css?v='.CSSVERSION.'" rel="stylesheet" type="text/css" />';
...
//if($bootstrapjs<>'') echo '<script src="'.$bootstrapjs.'" type="text/javascript"></script>';
  ...
  echo '<script src="'.TPLBASEPATH.'assets/vendor/jquery.js" type="text/javascript"></script>';
