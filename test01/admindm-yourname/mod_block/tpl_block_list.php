...
if($pidcate<>'' && $type=='blockdh'){
  $catname = get_field(TABLE_CATE,'name',$pidcate,'pidname');
  if($catname=='noid') echo ' <strong style="color:red">分类标识不对</strong>';
  else {
      echo ' <a  target="_blank" href="../mod_node/mod_blockdh.php?lang='.LANG.'&pid='.$pidcate.'">'.spangray($catname).'</a>';
  }  
 }


 if($pidcate<>'' && $type=='node'){
  
    if(is_int(strpos($pidcate,'|'))){
        //echo $pidcate; 
        echo ' <strong style="color:red">多个分类</strong>';
    }
    else{
