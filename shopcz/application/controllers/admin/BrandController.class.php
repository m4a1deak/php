...
class BrandController extends BaseController{
	...
	public function insertAction(){
		...
		if ($filename = $upload->up($_FILES["logo"])){
			...
			if ($brandModel->insert($data)){//添加成功	
				...
				$this->jump("index.php?p=admin&c=brand&a=add","添加商品品牌失败",1);
			}
		}else{//失败
	...
	public function updateAction(){
		...
		if($brandModel->update($data)){
			$this->jump("index.php?p=admin&c=brand&a=index","更新成功",1);
		}else{
			$this->jump("index.php?p=admin&c=brand&a=edit&brand_id=".$data['brand_id'],"更新失败",1);
		}
	}
	//定义delete方法，完成品牌的删除
	public function deleteAction(){
		...
		if ($brandModel->delete($brand_id)){
			//成功的同时删除对应的图片
			@unlink($img);
			$this->jump("index.php?p=admin&c=brand&a=index","删除成功",1);
		}else{
			$this->jump("index.php?p=admin&c=brand&a=index","删除失败",1);
		}
	}
}